<template>
    <div>
        <input type="checkbox" v-model="preselect" true-value="true" false-value="false"/>
        {{ preselect }}
        {{active_questions}}
        {{selected_questions}}
        <h1>Annotation</h1>
            ...
            <b-row class="justify-content-md-center">
                <b-card-group deck>
                    <b-card header="Select similar question(s)">
                        <b-list-group>
                            <b-list-group-item v-for="(question, index) in ranked_questions" :key="index"
                                               @click="question_clicked(question, index)"
                                               :class="{'active': active_questions[index]}" button>
                    ...
                    <b-card header="Select answer(s)">
                        <b-list-group>
                            ...
                            <b-list-group-item v-for="(answer, index) in filtered_answers" :key="index"
                                               @click="answer_clicked(index)" :class="{'active': selected_answer == index}"
                                               button>
                                <div class="answer">
                                    <b>{{answer.aid}}</b>
                            ...
                            <b-list-group-item>
                                <div class="add_news_answer">
                                    ...
                                    <b-button class="add_answer_button" block variant="primary" @click="add_new_answer">Add new
                                        answer
                                    </b-button>
                                </div>
                            </b-list-group-item>
...
<script>
    ...
    export default {
        ...
        computed: {
            preselect: {
                get() {
                    return this.$store.state.preselect
                },
                set(value) {
